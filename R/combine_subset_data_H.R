...
#'
#' @description For a a given feature set and list of data files (of master data from several cohorts), create a combined master subset containing only observations relevant to the provided cohort empi list.
#'
#' @export
...
#' @param cohort_empi vector of all unique empis in cohort observation set (character vector)
#' @return
#' @examples

combine_subset_data <- function(file_source_list, feature_set, cohort_empi){
	...
	# subset combined datasets to only include observations from relevant empi / identifiers
	#-------------------------------------------------------------------------------#
	...
	if(feature_set %in% list_of_list_feature_set){
		subset_combined_dt <- mapply(function(dt_list, dt_name){
			...
			dt_list <- dt_list[empi %in% cohort_empi, ]
		...
		subset_combined_dt <- combined_dt[empi %in% cohort_empi, ]
	...
	if(feature_set == "dem"){
			no_dem <- nrow(cohort[!(empi %in% subset_combined_dt$empi), ])
			if(no_dem > 0){
				print(sprintf("WARNING - COMBINED DEMOGRAPHICS FILE MISSING DATA ON %d empis", no_dem))
			} else{
