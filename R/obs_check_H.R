...
obs_check <- function(dt, cohort_key_var_list=cohort_key_var, 
  ...
  # generaet & print overview statistics
  ps(paste0("number of rows: %f, number of complete cases: %f, numer of complete cases as a perc of all rows: %f,",
    "number of columns: %d, number of features: %d, number of features excl. timeframe max: %d,", 
    "number of unique features exl.timeframe max: %d"), 
    nrow(dt), 
    ...
    length(unique(gsub("_$", "", gsub(timepattern_comb, "", names(dt[, mget(setdiff(names(dt),
      c(cohort_key_var_list,  cohort_extra_var_list, grep("_max$|_max_diff$",names(dt),value=T))))]))))))
